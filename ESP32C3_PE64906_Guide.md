...
#define PIN_SEN   2   // Serial Enable — manual chip select
#define SPICLK    3   // Serial Clock
#define SPIMOSI   4   // Serial Data (MOSI)
#define SPIMISO  -1   // Not used

// SPI settings: 100kHz, MSB first, Mode 0 (CPOL=0, CPHA=0)
SPISettings dtcSettings(100000, MSBFIRST, SPI_MODE0);

void setCapacitance(uint8_t state) {
    // Clamp to 5-bit range (0–31)
    // PE64906 register: [0][0][STB=0][d4..d0]
    if (state > 31) state = 31;

    SPI.beginTransaction(dtcSettings);   // Lock SPI bus + apply settings

    digitalWrite(PIN_SEN, LOW);   // Step 1: SEN idle / setup
    delay(1);
    digitalWrite(PIN_SEN, HIGH);  // Step 3: open data window
    delay(1);

    SPI.transfer(state);          // Step 5: clock 8 bits into DTC
    delay(1);

    digitalWrite(PIN_SEN, LOW);   // Step 7: LATCH — DTC captures value
...
void setup() {
    ...
    SPI.begin(SPICLK, SPIMISO, SPIMOSI, -1);
    digitalWrite(PIN_SEN, HIGH);  // SEN idle state
    delay(100);

    setCapacitance(15);           // ← Set your fixed state here
    Serial.println("Done. Cap set to state 15 (~3.0 pF).");
}

void loop() {
    // Empty — holds the fixed state set in setup()
}
```
